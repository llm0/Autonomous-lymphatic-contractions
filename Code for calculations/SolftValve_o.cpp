...
/*void SolftValve::Draw(CDC* pDC, CRect rect, double k)
{
Draw(&m_MemDL, pDC, rect, k);
Draw(&m_MemUL, pDC, rect, k);
Draw(&m_MemDR, pDC, rect, k);
Draw(&m_MemUR, pDC, rect, k);
}
void SolftValve::Draw(MemValve * mem,CDC* pDC, CRect rect, double k)
{
int i;
pDC->MoveTo(rect.left + int(mem->m_Seg[0]->m_pst.m_dx*k),
rect.bottom - int(mem->m_Seg[0]->m_pst.m_dy*k));
for (i = 0; i < VALSEGNUM - 1; i++)
{
pDC->LineTo(rect.left + int(mem->m_Seg[(i + 1) % VALSEGNUM]->m_pst.m_dx*k),
rect.bottom - int(mem->m_Seg[(i + 1) % VALSEGNUM]->m_pst.m_dy*k));
}
}*/
